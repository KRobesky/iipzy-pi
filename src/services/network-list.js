// copied from: https://www.npmjs.com/package/network-list
...
function getDeviceInfo_helper(result, ip, callback) {
  result.alive = true;
  dns.reverse(ip, function(err, host) {
    ...
    arp.getMAC(ip, function(err2, mac) {
      ...
      log("...getMac: ip = " + ip + ", mac = " + result.mac, "nlst", "info");
      if (options.vendor && mac) {
        request.get(
          "https://macvendors.co/api/" +
            mac.replace(/:([^:]{1}):/g, ":0$1:") +
            "/json",
          function(err3, httpRes, body) {
...
function getInfo(ip, callback) {
  ...
  ping.promise
    ...
    .then(function(res) {
      if (res.alive) {
        getDeviceInfo_helper(result, ip, callback);
        /*
        result.alive = true;
        dns.reverse(ip, function(err, host) {
          if (err) {
...
module.exports = {
  ...
  getDeviceInfo: function getDeviceInfo(ip, callback) {
    var result = {
      ip: ip,
      alive: false,
      hostname: null,
